...
class yabb26x_to_smf extends ConverterBase
{
	...
	public static function loadConverterSettings(): void
	{
		...
		static::$basepath = ($yabb['htmldir'] ?? '');
		static::$frompathappend = '/yabbfiles';
	}
		...
		while (true)
		{
			...
			while ($topic = ConverterDb::fetch_assoc($result))
			{
				...
				foreach ($messages as $message)
				{
					...
					if (isset($yabb['uploaddir']) && !empty($message[12]) && file_exists(self::checkAndFixPath($yabb['uploaddir'] . '/' . $message[12])))
						ConverterDb::insert(
							'{to_prefix}convert',
							['real_id', 'temp', 'type'],
	...
	public static function	 convertStep13Custom(): void
	{
		...
		while (true)
		{
			...
			while ($row = ConverterDb::fetch_assoc($result))
			{
				$size = filesize(self::checkAndFixPath($yabb['uploaddir'] . '/' . $row['temp_filename']));
				$file_hash = getAttachmentFilename($row['temp_filename'], $id_attach, null, true);
				$new_filename = $id_attach . '_' . $file_hash . '.dat';
				...
				if (strlen($row['temp_filename']) <= 255 && copy(self::checkAndFixPath($yabb['uploaddir'] . '/' . $row['temp_filename']), $attachmentUploadDir . '/' . $new_filename))
				{
					...
					{
						list ($width, $height) = getimagesize(self::checkAndFixPath($yabb['uploaddir'] . '/' . $row['temp_filename']));
